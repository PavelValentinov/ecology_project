...
# считываем с файла id компаний
file1 = open("DEL/only_id_test.csv", "r")
...
for line in lines:
	...
	# меняем headers при каждом 200 запросе
	if cikl % 200 == 0:
		user_agent = random.choice(user_agent_list)
		headers = {'User-Agent': user_agent, 'Accept-Encoding': 'identity'}
		print(f" Каждый 200-ый запрос = {cikl}")
		time.sleep(1)

	response_code_fkko = requests.get('https://rpn.gov.ru/licences/' + str(line.strip()) + '/', headers=headers)
	...
	def parser_fkko(pagination_end_fkko):
		...
		if pagination_end_fkko == 0:
			...
			# записываем пустые данные в файл  fkko_info.csv

			with open('DEL/fkko_test.csv', 'a', newline='', encoding="utf-8") as file:
		...
		# если даннные на странице пагинаций, проходим далее по всем страницам
			...
			while int(page_fkko) <= int(pagination_end_fkko):
				...
				# 	меняется header каждые 50 запросов
				if page_fkko % 50 == 0:
					user_agent = random.choice(user_agent_list)
					headers_fkko = {'User-Agent': user_agent, 'Accept-Encoding': 'identity'}
					print(f'User_agent = {headers_fkko}')
					print(f'page_fkko делиться на 25 = {page_fkko} из {pagination_end_fkko}')
					time.sleep(1)
				...
				try:
					...
					print(f'ошибка подключения к id {new_url_fkko_page}')
					time.sleep(1)
					continue
